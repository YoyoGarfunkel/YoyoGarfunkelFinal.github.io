  ...
  return declare([_WidgetBase, Evented], {
    ...
    **/
    _getGeodesicDistances: function (featureGeometry) {
      if (!this._selectedPoint || !this._selectedPoint.geometry || !featureGeometry) {
        return 0;
      }
      //pass the selected geometry, intersection geometry and configured unit
      return closestGeodesicDistanceUtils.getGeodesicDistance(this._selectedPoint.geometry,
        featureGeometry, this.config.bufferDistanceUnit.distanceUnit);
    },
      ...
      if (infos && infos.length > 0 ||
        ...
        this._checkMediaInfos(swiperWrapper, popupRenderer, addedURL).then(
          lang.hitch(this, function (hasMediaInfos) {
            ...
            this._getFeatureInfoLastNode();
            //Clear image handles so that the requests for the hidden mediaSection will no be fired
            if (popupRenderer._clearImageHandles) {
              popupRenderer._clearImageHandles();
            }
          }));
      } else {
    ...
    **/
    ...
    updateImageData: function (imageDiv) {
      fetch(imageDiv.alt, { cache: "no-cache" }).then(function (response) {
        //revokes the previous object
        URL.revokeObjectURL(imageDiv.src);
        response.blob().then(function (blobResponse) {
          imageDiv.setAttribute("src", URL.createObjectURL(blobResponse));
        });
      });
    },
    ...
    **/
    _attachEventOnImage: function (imageDiv, refreshIntervalTime) {
      ...
      if (refreshIntervalTime) {
        ...
        if (refreshIntervalTime && refreshIntervalTime > 0) {
          refreshTimer = setInterval(lang.hitch(this, function () {
            this.updateImageData(imageDiv);
          }), refreshIntervalTime);
    ...
    */
    _getPolyLineSymbol: function (graphic, layer) {
      ...
      if (graphic.geometry.paths && graphic.geometry.paths.length > 0) {
        array.forEach(graphic.geometry.paths, lang.hitch(this, function (eachPath) {
          polyline.addPath(eachPath);
        }));
      }
